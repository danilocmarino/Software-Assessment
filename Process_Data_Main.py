...
try:
    ...
    with open('Students1.csv', 'r') as file:
            ...
            for row in reader:#... it makes the names working throught a row.
                name, geography, history, portuguese = row #... subjects which needs to be calculated.
                average = Calculation_Functions.calculate_average([int(geography), int(history), int(portuguese)]) #....it calculates the average of these subjects.
                grade = Calculation_Functions.assign_grade(average) #... knowing the average. it works giving the grade
                Students.append([name, geography, history, portuguese, average, grade]) #... this command works using each students, average and grade.
...
with open('Student_Result.csv', 'w', newline='') as file: #...this command write everything on the result file.
     ...
     writer.writerow(["Name", "Geography", "History", "Portuguese", "Average", "Grade"])
